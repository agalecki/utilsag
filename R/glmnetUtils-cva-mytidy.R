...
#' @export
mytidy.cva.glmnet <- function(x, return_zeros = FALSE,  unnest_df = character(0), ...){
 ...
 funi <- function(i){
    ...
    tbl1 %>% print(n=1000)
    tbl2 <- tibble(alpha_idx = i, mytidy(fiti, return_zeros = return_zeros, unnest_df = unnest_df, ...))  
    if (i==1) {
     print(colnames(tbl1))
     print(colnames(tbl2))
    }
